...
// ✅ Initialize TTS client using ENV variable instead of keyFilename
let client;
try {
...
app.post('/api/tts/speak', async (req, res) => {
  if (!client) {
    return res.status(500).json({ error: 'TTS client not initialized. Check GOOGLE_TTS_KEY environment variable.' });
  }
