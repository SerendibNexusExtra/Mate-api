...
// Initialize TTS client with environment variable or file
let client;
...
try {
  if (process.env.GOOGLE_TTS_KEY_BASE64) {
    ...
    console.log('TTS client initialized with environment variable');
  } else {
    console.log('Attempting to load TTS key from file...');
    client = new textToSpeech.TextToSpeechClient({
      keyFilename: 'google-tts-key.json'
    });
  ...
  // Exit if TTS client fails to initialize (optional)
  // process.exit(1);
}
